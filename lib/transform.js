  ...
  return through(function write(data) {
    ...
    try {
      ...
      src.replace(['require'], function(node) {
        ...
        if (inline)
          node = readJSONNode(jsonFile)

        //We could use async & catch some errors here
        //and allow bundler to continue even without json-live
        var id = jsonFile
        announce({
          _id: id,
          path: jsonFile
...
function readJSONNode(file) {
  var jsonContents = fs.readFileSync(file, 'utf8')
  if (!jsonContents) 
      ...
      function(err, resp) {
        if (err) {
          if (err.message.indexOf('ECONNREFUSED'))
            console.error('Could not find json-live server on port', port)
